...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, past_kvs=None):
        ...
        # If using cache, only need position for new tokens
        if past_kvs is not None:
            pos = torch.arange(t, dtype=torch.long, device=device) + past_kvs[0][0].size(2)  # offset by cache length
        else:
        ...
        return logits, loss, present_kvs  # CHANGED: Return cache
    ...
    def generate_with_cache(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        """Generate with KV cache for speedup"""
        past_kvs = None
